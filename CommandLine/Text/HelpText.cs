...
namespace CommandLine.Text
{
    ...
    public class HelpText
    {
        ...
        public static HelpText AutoBuild<T>(ParserResult<T> parserResult)
        {
            ...
            if (errors.Any(e => e.Tag == ErrorType.BadVerbSelectedError))
            {
                return AutoBuild(parserResult, current => DefaultParsingErrorsHandler(parserResult, current), e => e, true);
            }
        ...
        private HelpText AddOption(string requiredWord, int maxLength, Specification specification, int widthOfHelpText)
        {
            ...
            if (addEnumValuesToHelpText && specification.EnumValues.Any())
                optionHelpText += " Корректные значения: " + string.Join(", ", specification.EnumValues);

            specification.DefaultValue.Do(
                defaultValue => optionHelpText = "(По умолчанию: {0}) ".FormatInvariant(FormatDefaultValue(defaultValue)) + optionHelpText);
